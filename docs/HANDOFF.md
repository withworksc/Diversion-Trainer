...
- **東部改降高雄的「另一組答案」（v2.2.2a）**：直飛（地圖紅線）不動，另外給一組
  ...
  **恆春到高雄這段直線會經過外海的 RCR34 限航區（SFC–14,000 ft）**：在航圖上量過，圓心約
  ...
  使用者說「高雄西部可以直接切直線」，改成直線；答案原本留了一句 RCR34 的提醒，使用者決定
  拿掉，所以現在答案完全不提 RCR34。`compute()` 裡直飛與另一組共用 `routeCalc()`，`r.alt` 只有改降高雄時才有；答案
...
13. **高雄另一組答案的高度**（v2.2.2a）：寫 3,000 ft 以下，跟直飛高雄一樣，沒有確認過。
   恆春到高雄的直線會經過 RCR34（SFC–14,000 ft），使用者決定答案不提。
