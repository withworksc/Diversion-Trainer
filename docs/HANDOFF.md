...
- **不動桿的漂移(60 Hz,60 顆種子的中位數)**:v2.0.1 照使用者要求取 v1 跟 v2 的中間
  (`pitch.gustJitter` 1.6 → 1.2、`roll.gustJitter` 7 → 4);v2.0.2 使用者要垂直的亂流
  再低一點點(`pitch.gustJitter` 1.2 → 1.05,roll 不動):

  | | v1 | v2 | v2.0.1 | v2.0.2 |
  |---|---|---|---|---|
  | pitch 偏離平飛 RMS | 約 1.3° | 約 2.6° | 約 2.0° | 約 1.75° |
  | 坡度,10 秒 | 約 0.7° | 約 3° | 約 1.8° | 約 1.8° |
  | 坡度,30 秒 | 約 0.8° | 約 6°(最差一成 17°) | 約 3.5°(最差一成 10°) | 同 v2.0.1 |
