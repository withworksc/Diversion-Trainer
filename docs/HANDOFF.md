...
- **東部改降高雄的「另一組答案」（v2.2.2a）**：直飛（地圖紅線）不動，另外給一組先回恆春、
  再沿 C9 西岸走廊到高雄的答案（`Scenario.altRoute`，地圖橘線 `Map.ALT_COLOR`，有圖例）。
  航路是 位置 → 恆春 → 楓港 → 枋寮 → 東港 → 高雄，照 `CORRIDORS.C9` 的定義；三個 C9 報告點
  （`Data.C9`）在航圖上量三角形中心。**不能從恆春直線到高雄**：那條線會穿進外海的 RCR34
  限航區（SFC–14,000 ft）約 5 NM（`Data.RESTRICTED` 有圓心半徑，測試會檢查）；C9 是貼著
  RCR34 東緣畫的，各段頂多貼邊（在量測誤差 1 NM 內）。`compute()` 裡直飛與另一組共用
  `routeCalc()`，`r.alt` 只有改降高雄時才有；答案第 3–7 格底下各加一段橘色邊線的區塊。
  目前只有高雄有另一組，其他目的地要加，改 `altRoute()`。
...
12. **港仔鼻題型**（v2.2.2a）：「恆春外海」我取的是半島**西岸**外海（從那裡直切港仔鼻才需要
   ...
   要改 `makeScenario`。
13. **高雄另一組答案經過的點**（v2.2.2a）：使用者說「先到恆春再到高雄」，我照 C9 的定義補了
   楓港、枋寮、東港三個點（直線會穿過 RCR34）。另一組的高度寫 3,000 ft 以下，跟直飛高雄一樣，
   沒有確認過 C9 自己的高度規定。
